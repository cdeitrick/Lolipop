...
class OutputFilenames:
	...
	def __init__(self, output_folder: Path, name: str, delimiter = '\t'):
		...
		if not subfolder.exists():
			subfolder.mkdir()
		self.original_trajectory: Path = subfolder / (name + '.trajectories.original.tsv')
		self.original_genotype: Path = subfolder / (name + '.muller_genotypes.original.tsv')
		self.trajectory: Path = output_folder / (name + '.trajectories.tsv')
		self.genotype: Path = output_folder / (name + '.muller_genotypes.tsv')
		self.population: Path = output_folder / (name + '.ggmuller.populations.tsv')
		self.edges: Path = output_folder / (name + '.ggmuller.edges.tsv')
		self.r_script: Path = subfolder / (name + '.r')
		self.muller_table: Path = subfolder / (name + '.muller.csv')
		...
		self.p_value_matrix:Path = subfolder / (name + ".pvalues.matrix.tsv")
