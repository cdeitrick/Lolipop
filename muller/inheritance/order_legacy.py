...
def order_clusters(sorted_df: pandas.DataFrame, options: OrderClusterParameters) -> Cluster:
	...
	additive_background_double_cutoff  = OrderClusterParameters.additive_background_double_cutoff
	additive_background_single_cutoff = OrderClusterParameters.additive_background_single_cutoff
	subtractive_background_double_cutoff = OrderClusterParameters.subtractive_background_double_cutoff
	subtractive_background_single_cutoff = OrderClusterParameters.subtractive_background_single_cutoff
	derivative_detection_cutoff = OrderClusterParameters.derivative_detection_cutoff
	derivative_check_cutoff = OrderClusterParameters.derivative_check_cutoff
	new_background_base_cutoff = OrderClusterParameters.new_background_base_cutoff
	new_background_significant_cutoff = OrderClusterParameters.new_background_significant_cutoff
	...
	for unnested_label, unnested_trajectory in sorted_df[1:].iterrows():
		...
		for nested_label, row in table_of_checks.iterrows():
			...
			logger.info(f"Computed priority: {priority}")
			logger.info(f"{unnested_label}|{nested_label} Full Check: {full_check}")
			logger.info(f"{unnested_label}|{nested_label} Area Check: {area_check} ({area_ratio})")
			logger.info(f"{unnested_label}|{nested_label} Additive Check: {additive_check}")
			logger.info(f"{unnested_label}|{nested_label} Derivative check: {derivative_check}")
			logger.info(f"{unnested_label}|{nested_label} Area Difference: {area_difference}")
			...
			if area_check and delta > options.derivative_check_cutoff and additive_check:
				...
				logger.info(f"Complete Check: True")
